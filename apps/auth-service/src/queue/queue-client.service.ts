...
import { WelcomeEmailJob, PasswordResetEmailJob, CreateUserJob } from '@hic/shared-dto';
...
export class QueueClientService {
  ...
  async sendWelcomeEmail(data: WelcomeEmailJob): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to queue welcome email for ${data.email}:`, error);
      throw error;
    }
  }
  ...
  async sendPasswordResetEmail(data: PasswordResetEmailJob): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to queue password reset email for ${data.email}:`, error);
      throw error;
    }
  }
  ...
  async sendCreateUserJob(data: CreateUserJob): Promise<void> {
    try {
      ...
      this.logger.error(`Failed to queue create user job for ${data.email}:`, error);
      throw error;
    }
  }
}