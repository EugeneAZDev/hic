...
import { Register, AuthResponse, TokenResponse, PublicUser } from '@hic/shared-dto';
...
export class AuthService {
  constructor(
    ...
    console.log('AuthService: JWT_SECRET loaded:', this.configService.get<string>('JWT_SECRET') ? 'YES' : 'NO');
    console.log('AuthService: JWT_SECRET length:', this.configService.get<string>('JWT_SECRET')?.length || 0);
  }
  ...
  async validateUser(email: string, password: string): Promise<PublicUser | null> {
    ...
    if (authUser && authUser.passwordHash && await bcrypt.compare(password, authUser.passwordHash)) {
