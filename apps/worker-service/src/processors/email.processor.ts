...
export class EmailProcessor {
  ...
  async handleWelcomeEmail(job: Job<WelcomeEmailJob>) {
    this.logger.log(`Processing welcome email for ${job.data.email} (Job ID: ${job.id})`);
    
    try {
      await this.emailService.sendWelcomeEmail(job.data.email, job.data.name);
      this.logger.log(`Welcome email processed successfully for ${job.data.email} (Job ID: ${job.id})`);
    } catch (error) {
      this.logger.error(`Failed to send welcome email to ${job.data.email} (Job ID: ${job.id}):`, error);
      
      // Don't throw error for authorization issues in development
      if (error.response?.status === 401) {
        this.logger.warn('Welcome email sending failed due to authorization. This is likely due to IP restrictions in Sendinblue.');
        this.logger.warn('Email will be skipped but user registration will continue.');
  ...
  async handlePasswordResetEmail(job: Job<PasswordResetEmailJob>) {
    ...
    try {
      ...
      this.logger.log(`Password reset email sent successfully to ${job.data.email}`);
    } catch (error) {
      this.logger.error(`Failed to send password reset email to ${job.data.email}:`, error);
      throw error;
    }
  }
  ...
  async handleCustomEmail(job: Job<CustomEmailJob>) {
    ...
    try {
      ...
      this.logger.error(`Failed to send custom email to ${job.data.to}:`, error);
      throw error;
    }
  }
