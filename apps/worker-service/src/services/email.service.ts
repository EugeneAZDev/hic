...
export class EmailService {
  ...
  constructor(private configService: ConfigService) {
    ...
    if (apiKey) {
      ...
      if (!apiKey.startsWith("xkeysib-") || apiKey.length < 50) {
        this.logger.warn(
          "BREVO_API_KEY appears to be invalid or malformed. Email sending will be disabled.",
        );
        this.logger.warn(
          'Valid Brevo API keys should start with "xkeysib-" and be at least 50 characters long.',
        );
        return;
      }
      ...
      this.logger.warn(
        "BREVO_API_KEY not configured. Email sending will be disabled.",
      );
    }
  }
  ...
  async sendEmail(data: CustomEmailJob): Promise<void> {
    ...
    const useMailHog =
      this.configService.get<string>("USE_MAILHOG", "false") === "true";
    ...
    if (isDevelopment) {
      ...
      if (data.templateId) {
        ...
        this.logger.log(
          `Template Params: ${JSON.stringify(data.params || {}, null, 2)}`,
        );
      } else {
        if (data.htmlContent) {
    ...
    try {
      ...
      if (error.response?.status === 401 || error.response?.status === 403) {
        this.logger.error(
          "Email sending failed due to authorization issues in production!",
        );
        throw error;
      }
  ...
  private async sendEmailViaMailHog(data: CustomEmailJob): Promise<void> {
    try {
      ...
      this.logger.error(
        `Failed to send email via MailHog to ${data.to}:`,
        error,
      );
